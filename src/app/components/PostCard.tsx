  ...
  return (
    <div className="bg-white rounded-xl mb-4 shadow-sm border border-gray-50 overflow-hidden">
      ...
      <Link to={`/post/${post.id}`} className="block relative w-full aspect-square bg-gray-50 overflow-hidden">
        <div className="flex w-full h-full overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]" onClick={handleDoubleTap}>
          {post.images.map((img: string, idx: number) => (
            <div key={idx} className="w-full h-full shrink-0 snap-center relative">
              <ImageWithFallback src={img} alt={`Post image ${idx + 1}`} className="w-full h-full object-cover"/>
              ...
              {/* Double-tap heart animation - only on first image */}
              {idx === 0 && showHeart && (
      ...
      <div className="flex items-center justify-between p-3 pb-2">
        <div className="flex items-center gap-4">
          ...
          <button onClick={onShare} className="flex items-center gap-1.5 text-gray-600">
...
export function PostCardSkeleton() {
  return (
    <div className="bg-white rounded-xl mb-4 shadow-sm border border-gray-50 overflow-hidden animate-pulse">
      ...
      <div className="flex gap-6 p-3 pb-0">
