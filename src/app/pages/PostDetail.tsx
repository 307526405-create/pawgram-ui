import { ChevronLeft, Heart, Share2, Send } from "lucide-react";
...
export function PostDetail() {
  ...
  useEffect(() => {
    if (!id) return;
    postsApi.detail(Number(id)).then(p => { setPost(p); setLoading(false); }).catch(() => setLoading(false));
  }, [id]);
      ...
      <div className="flex-1 overflow-y-auto pb-[150px] [&::-webkit-scrollbar]:hidden">
            ...
            {(post.images||[]).map((img: string, idx: number) => (
              <div key={idx} className="w-full h-full shrink-0 snap-center relative">
                ...
                {(post.images||[]).length > 1 && (
                  <div className="absolute top-4 right-4 bg-black/40 text-white text-[12px] px-2 py-0.5 rounded-full">{idx+1}/{post.images.length}</div>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="bg-white px-4 pt-4 pb-2">
          ...
          <div className="flex items-center justify-between py-2 mb-2">
            ...
            <button onClick={handleShare} className="text-[#666]"><Share2 className="w-5 h-5"/></button>
          </div>
        </div>
