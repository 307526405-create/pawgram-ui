    ...
    return (
      <div className="h-full bg-[#FAFAFA] flex flex-col">
        <div className="pt-[var(--app-safe-top)] h-[var(--app-header-height)] flex items-center justify-center shrink-0">
          <h1 className="text-[17px] font-bold text-[#333]">{t('profile.title') || '我的'}</h1>
        </div>
        <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center px-8">
          ...
          <p className="text-[13px] text-[#999] mb-8">{t('login.tagline')}</p>

          <div className="w-full max-w-[320px] space-y-3">
            <div className="bg-[#F5F5F5] rounded-xl h-[50px] flex items-center px-4">
              <span className="text-[14px] text-[#999] mr-2">+86</span>
              <div className="w-px h-5 bg-[#DDD] mr-3"/>
              <input type="tel" placeholder={t('login.enterPhone')} value={phone}
                onChange={(e) => { setPhone(e.target.value); setError(''); }}
                className="flex-1 bg-transparent text-[15px] outline-none" />
            </div>
            <div className="flex gap-2">
              <input type="tel" maxLength={6} inputMode="numeric" placeholder={t('login.enterCode')} value={code}
                onChange={(e) => { handleCodeChange(e.target.value); }}
                className="w-[120px] bg-[#F5F5F5] rounded-xl h-[50px] px-3 text-[15px] text-center tracking-[8px] font-bold outline-none shrink-0" />
              <button onClick={handleSendCode} disabled={countdown > 0 || !phone}
                className="flex-1 h-[50px] bg-[#FF8C42] rounded-xl text-white text-[14px] font-bold active:bg-[#E67A35] transition-colors disabled:opacity-50">
          ...
          <div className="mt-8 w-full max-w-[320px]">
            <div className="flex items-center gap-4 mb-5">
              <div className="flex-1 h-[0.5px] bg-[#E5E5E5]"/>
              <span className="text-[12px] text-[#BBB]">{t('login.otherLogin')}</span>
              <div className="flex-1 h-[0.5px] bg-[#E5E5E5]"/>
            </div>
            <div className="flex justify-center gap-8">
              ...
              <button onClick={handleGoogleLogin} className="w-[48px] h-[48px] rounded-full border-2 border-[#E5E5E5] flex items-center justify-center active:scale-95 transition-transform shadow-sm">
