...
export function Profile() {
  ...
  const handleSendCode = async () => {
    if (loginMode === 'phone' && (!phone || countdown > 0)) return;
    if (loginMode === 'email' && (!email || countdown > 0)) return;
    try {
      if (loginMode === 'phone') await authApi.sendCode(phone);
      else await authApi.sendEmailCode(email);
      setCountdown(60);
  ...
  if (!loggedIn) {
    return (
      <div className="h-full bg-[#FAFAFA] dark:bg-gray-950 flex flex-col">
        <div className="pt-[var(--app-safe-top)] h-[var(--app-header-height)] flex items-center justify-center shrink-0">
          <h1 className="text-[17px] font-bold text-[#333] dark:text-gray-100">登录</h1>
        </div>
        <div className="flex-1 overflow-y-auto flex flex-col items-center justify-center px-8">
          <img src="/app-icon.png" className="w-[72px] h-[72px] rounded-2xl mb-4 shadow-lg shadow-[#FF8C42]/30" alt=""/>
          <h2 className="text-[22px] font-bold text-[#FF8C42] mb-1">{t('login.brandName')}</h2>
          <p className="text-[13px] text-[#999] dark:text-gray-400 mb-8">{t('login.tagline')}</p>

          <div className="w-full max-w-[320px] space-y-3">
            {loginMode === 'phone' ? (
              <>
                <div className="bg-[#F5F5F5] dark:bg-gray-800 rounded-xl h-[50px] flex items-center px-4">
                  <span className="text-[15px] text-[#999] dark:text-gray-400 mr-2">+86</span>
          ...
          <div className="mt-8 w-full max-w-[320px]">
            <div className="flex items-center gap-4 mb-5">
              ...
              <span className="text-[12px] text-[#BBB] dark:text-gray-500">{t('login.otherLogin')}</span>
              <div className="flex-1 h-[0.5px] bg-[#E5E5E5] dark:bg-gray-700"/>
            </div>
            <div className="flex justify-center gap-8">
              ...
              <button onClick={handleAppleLogin} className="w-[48px] h-[48px] rounded-full bg-black dark:bg-gray-100 flex items-center justify-center active:scale-95 transition-transform shadow-sm">
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-white dark:fill-black"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
              </button>
              ...
              {loginMode === 'phone' ? (
                <button onClick={() => setLoginMode('email')} className="w-[48px] h-[48px] rounded-full bg-[#FF8C42] flex items-center justify-center active:scale-95 transition-transform shadow-sm">
                  <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="white" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 4L12 13 2 4"/></svg>
                </button>
              ) : (
                <button onClick={() => setLoginMode('phone')} className="w-[48px] h-[48px] rounded-full bg-[#FF8C42] flex items-center justify-center active:scale-95 transition-transform shadow-sm">
          ...
          <p className="mt-8 text-center text-[12px] text-[#BBB] dark:text-gray-500">
